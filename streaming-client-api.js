...
fetch('/api/keys')
  ...
  .then(async (config) => {
    DID_API = { key: config.DID_API_KEY, url: 'https://api.d-id.com' };
...
import { fetchOpenAIAssistantResponse } from './utils/openai-assistant.js';
...
connectButton.onclick = async () => {
  ...
  const sessionResponse = await fetch(`${DID_API.url}/talks/streams`, {
    ...
    body: JSON.stringify({
      // source_url: "https://i.postimg.cc/9f5vjHdD/jimmy-fermin-bqe0-J0b26-RQ-unsplash.jpg",
      source_url: "https://i.postimg.cc/fLSRyN3Y/oracle-pic.png",
...
talkButton.onclick = async () => {
  if (peerConnection?.signalingState === 'stable' || peerConnection?.iceConnectionState === 'connected') {
    ...
    const responseFromOpenAI = await fetchOpenAIAssistantResponse(userInput, OPENAI_API_KEY, ASSISTANT_ID);
    //
...
function playIdleVideo() {
  ...
  talkVideo.src = 'george_washington/oracle_idle.mp4';
  talkVideo.loop = true;
}
